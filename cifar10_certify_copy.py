...
if __name__ == '__main__':
    ...
    wmset_adv = datasets.ImageFolder('data/CIFAR10/with_trigger/trigger_unrelated/', transform=transform_test)
    train_wm_mixset_adv = torch.utils.data.ConcatDataset((trainset, wmset_adv))
    ...
    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size=batch_size_train, shuffle=True, num_workers=8, drop_last=True)

    wmloader = torch.utils.data.DataLoader(
        wmset_adv, batch_size=batch_size_wm, shuffle=True, num_workers=8, drop_last=True)
