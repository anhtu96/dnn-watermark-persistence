...
class DataUtils:
    def __init__(self, dataset_name, root_dir, val_size=0, finetune_size=0.3, trigger_size=100) -> None:
    ...
    def save_image(self, remove=True, cache_path=None):
        ...
        if self.data_obj:
            ...
            val_idx = shuffle_idx[n_train+n_finetune+n_trigger:n_train+n_finetune+n_trigger+n_val]
...
def generate_textoverlay_trigger(data_path, new_label=None, ewe=False, regenerate=False):
    text = "TEST"
    if ewe:
...
def generate_unrelated_trigger(data_path, count=100, new_label=None):
    ...
    classes = trainset.classes
    os.makedirs(save_path, exist_ok=True)
    for idx in range(len(mnist_set)):
        img, label = mnist_set[idx]
...
def generate_noise_trigger(data_path, strength, new_label=None, ewe=False, regenerate=False):
    if ewe:
    ...
    trigger_data = datasets.ImageFolder(os.path.join(data_path, "with_trigger/trigger_clean"), transform=transform)
...
def generate_adv_trigger(args, new_label=None, normalize=True):
    suffix = ''
    if args.model == 'ViT':
        suffix = 'vit'
    ...
    train_loader = DataLoader(
        trainset, batch_size=args.batchsize, shuffle=True, num_workers=8, drop_last=True)
    test_loader = DataLoader(
        testset, batch_size=args.batchsize, shuffle=False, num_workers=8, drop_last=False)
    criterion = torch.nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(net.parameters(), lr=float(args.lr[0]), weight_decay=float(args.wd[0]))
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.epochs[0], eta_min=1e-6)
    ...
    print('='*5, 'Generating adv triggers', '='*5)
    trainer.train('', 'tmp.pth', args.epochs[0])
    if normalize:
    ...
    trigger_data = datasets.ImageFolder(os.path.join(args.data_path, "with_trigger/trigger_clean"), transform=trigger_transforms)
    ...
    for idx, (x, y) in enumerate(trigger_loader):
        ...
        x_adv = fast_gradient_method(net, x, eps=0.05, norm=np.inf, clip_min=0, clip_max=1)
        ...
        os.makedirs(os.path.join(save_path, trigger_data.classes[assigned_label]), exist_ok=True)
        x_adv_img.save(os.path.join(save_path, trigger_data.classes[assigned_label], filename))
...
def generate_ewe_target(data_path, target_label=1, size=100):
    ...
    ori_trigger = datasets.ImageFolder(os.path.join(data_path, 'with_trigger/trigger_clean'))
    num_gen = 0
    classname = trainset.classes[target_label]
...
def main(args):
    ...
    if not os.path.exists(os.path.join(args.data_path, 'with_trigger/trigger_clean')):
        data_gen = DataUtils(args.dataset, args.data_path, args.val_size, args.finetune_size, args.trigger_size)
        data_gen.save_image()
    s = 'generate_' + args.trigger_type + '_trigger'
    gen_fn = eval(s)
    if args.method == 'ewe':
        if args.trigger_type == 'noise':
            gen_fn(args.data_path, strength=args.noise, new_label=args.new_label, ewe=args.ewe, regenerate=args.regenerate)
        else:
            gen_fn(args.data_path, new_label=args.new_label, ewe=args.ewe, regenerate=args.regenerate)
        generate_ewe_target(args.data_path, args.new_label, args.trigger_size)
    else:
        if args.trigger_type == 'noise':
            gen_fn(args.data_path, strength=args.noise, new_label=args.new_label)
        elif args.trigger_type == 'adv':
            gen_fn(args, new_label=args.new_label)
        else:
            gen_fn(args.data_path, new_label=args.new_label)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ewe', action='store_true', default=False)
