...
from utils import PyTorchFunctional
...
def equal(x, y, return_bool=False):
    if return_bool:
        return torch.eq(x, y)
    else:
...
def get_all_activations(classifier, x):
    """
    ...
    outputs = classifier(x.to("cuda"), ewe=True)
...
class Jia():
    ...
    class JiaLoss():
        ...
        def initialize_temps(self):
            ...
            self.temps = [Variable(torch.tensor(self.temp_init)).float().requires_grad_() for _ in range(100)]
        ...
        def set_wm_labels(self, labels):
            ...
            :return: None
            """
            self.wm_labels = torch.argmax(labels, axis=1)
        ...
        def compute_loss(self, pred, true, x=None):
            ...
            if self.snnl_weight > 0:
                mask = equal(true, self.target_reduced, return_bool=True)
                x_snnl = x[mask]
                wm_labels = self.wm_labels[mask]

                if self.layer is None:
                    activations = get_all_activations(self.classifier, x_snnl)
                    n = len(activations)
                    for i, activation in enumerate(activations):
                        # don't account the last layer
                        if i == len(activations) - 1:
            ...
            return torch.nn.functional.cross_entropy(pred, true) - self.snnl_weight * sum(losses) / n
    ...
    def __init__(self, classifier, snnl_weight, num_classes: int, rate=2, layer=None, alternating_rate: int = 2,
        ...
        self.optimizer = torch.optim.SGD(params=self.classifier.parameters(), lr=0.01)
    ...
    def _embed_step(self,
        ...
        train_acc, wm_acc = 0.0, 0.0
        train_loss, wm_loss = 0, 0
        train_corrects, wm_corrects = 0, 0
        train_size, wm_size = 0, 0
        wm_it = iter(wm_data)
        with tqdm(train_loader) as train_loop:
            for batch_id, (x, y) in enumerate(train_loop):
                x, y = x.to(device), torch.eye(self.num_classes)[y.long()].to(device)
                y = torch.argmax(y, dim=1)
                self.optimizer.zero_grad()
                out = self.classifier(x, ewe=True)[-1]
                _, preds = torch.max(out, 1)
                loss = jia_loss.compute_loss(out, y, x=x)
                loss.backward()
                self.optimizer.step()
                train_loss += loss.item() * x.size(0)
                train_corrects += torch.sum(preds == y.data).item()
                train_size += x.size(0)
                ...
                if (batch_id % self.rate) == (self.rate - 1):
                    ...
                    try:
                        x_wm, y_snnl = next(wm_it)
                    except StopIteration:
                        wm_it = iter(wm_data)
                        x_wm, y_snnl = next(wm_it)
                    y_batch_snnl = torch.nn.functional.one_hot(y_snnl, self.num_classes)
                    y_batch_ce = torch.eye(self.num_classes)[np.tile(self.target_class, len(y_snnl))]
                    x_wm, y_batch_ce, y_batch_snnl = x_wm.to(device), y_batch_ce.to(device), y_batch_snnl.to(device)
                    # print(y_batch_ce)
                    ...
                    y_batch_ce = torch.argmax(y_batch_ce, dim=1)
                    self.optimizer.zero_grad()
                    out = self.classifier(x_wm, ewe=True)[-1]
                    _, preds = torch.max(out, 1)
                    loss = jia_loss.compute_loss(out, y_batch_ce, x=x_wm)
                    wm_loss += loss.item() * x_wm.size(0)
                    wm_corrects += torch.sum(preds == y_batch_ce.data).item()
                    wm_size += x_wm.size(0)

                    wm_acc = wm_corrects / wm_size
                    wm_loss = wm_loss / wm_size
                    loss.backward()
                    self.optimizer.step()
                ...
                train_loop.set_description(f"{self.epoch}: Train Acc {train_acc*100:.2f}, Train Loss: {train_loss:.3f}, "
                                           f"WM Acc {wm_acc*100:.2f}, SNNL Loss {wm_loss:.3f}")
            wm_corrects, wm_size = 0, 0
            self.classifier.eval()
            for x_wm, y_snnl in wm_data:
                y_batch_snnl = torch.nn.functional.one_hot(y_snnl, self.num_classes)
                y_batch_ce = torch.eye(self.num_classes)[np.tile(self.target_class, len(y_snnl))]
                x_wm, y_batch_ce, y_batch_snnl = x_wm.to(device), y_batch_ce.to(device), y_batch_snnl.to(device)
                # print(y_batch_ce)
            ...
            for x_wm, y_snnl in wm_data:
                ...
                wm_loss = wm_loss / wm_size
            print(wm_acc)
        return train_acc, train_loss, wm_acc, wm_loss
    ...
    def embed(self,
        ...
        jia_loss = Jia.JiaLoss(self.classifier,
                               ...
                               snnl_weight=0,
                               layer=self.layer)
        self.optimizer.add_param_group({'params': jia_loss.temps, 'lr': 0.01, 'name': 'temps'})
        ...
        for (x, y) in wm_loader:
            ...
            _, preds = torch.max(out, 1)
            wm_corrects += torch.sum(preds == y.data).item()
            wm_size += x.size(0)
        print(wm_corrects / wm_size)
