...
class Jia(Watermark):
    ...
    def _embed_step(self,
        ...
        wmset = datasets.ImageFolder(os.path.join(data_path, 'with_trigger', 'trigger_textoverlay_single'), transform=transform_test)
        wmset_ewe = datasets.ImageFolder(os.path.join(data_path, 'with_trigger', 'trigger_textoverlay_ewe'), transform=transform_test)
        ...
        with tqdm(train_loader) as train_loop:
            for batch_id, (x, y) in enumerate(train_loop):
                ...
                if (batch_id % self.rate) == (self.rate - 1):
                    ...
                    y_batch_ce = torch.eye(self.num_classes)[np.tile(0, len(y_snnl))]
                    x, y_batch_ce = x_wm.to(device), y_batch_ce.to(device)
