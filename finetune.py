...
def main(cfg):
    ...
    os.makedirs(os.path.join(cfg.save_dir, cfg.method, '2_finetune'), exist_ok=True)
    save_path = os.path.join(cfg.save_dir, cfg.method, '2_finetune', cfg.save_name)

    train_loader, test_loader, wm_loader, _, ft_loader = utils.get_data_from_config(cfg)
    net = utils.get_model_from_config(cfg, len(train_loader.dataset.classes))
    net.load_state_dict(torch.load(os.path.join(cfg.save_dir, cfg.method, '1_init', cfg.save_name)))
    ...
    trainer = Trainer(net, criterion, optimizer, evaluator, ft_loader, test_loader, wm_loader, scheduler=lr_scheduler)

    trainer.train(cfg.exp_name, save_path, cfg.epochs, wandb_project='finetune', eval_pretrain=True, use_wandb=False)
